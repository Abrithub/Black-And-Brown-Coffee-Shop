...
const CustomerAuth = ({ onClose }) => {
    ...
    return (
        ...
        >
            <div
                className="bg-gradient-to-br from-[#2A1A10] to-[#1a0f0a] rounded-3xl p-8 w-full max-w-md border border-[#C19976]/30 shadow-2xl relative"
                role="document"
            >
                {/* Close Button */}
                <button
                ...
                <div className="text-center mb-8">
                    <h2 id="customer-auth-title" className="text-3xl font-bold text-[#C19976] mb-2">
                ...
                {error && (
                    <div className={`rounded-lg p-3 mb-6 ${error.includes('New user detected')
                        ...
                        <p id="auth-error" className={`text-sm ${error.includes('New user detected') ? 'text-blue-400' : 'text-red-400'
                            }`}>{error}</p>
                    </div>
                )}

                {/* Form */}
                <form onSubmit={handleSubmit} className="space-y-6" aria-describedby={error ? 'auth-error' : undefined}>
                    {!isLogin && (
                        <div>
